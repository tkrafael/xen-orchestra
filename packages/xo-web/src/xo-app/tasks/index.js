...
import { differenceBy, flatMap, flatten, groupBy, isEmpty, keys, map, some, toArray } from 'lodash'
import {
  createFilter,
  createGetObject,
...
export default class Tasks extends Component {
  ...
  UNSAFE_componentWillReceiveProps(props) {
    const finishedTasks = differenceBy(
      flatten(toArray(this.props.resolvedPendingTasksByPool)),
      flatten(toArray(props.resolvedPendingTasksByPool)),
      'id'
    )
  ...
  _getTasks = createSelector(
    ...
    (poolIds, resolvedPendingTasksByPool) =>
      isEmpty(poolIds)
        ? flatten(toArray(resolvedPendingTasksByPool))
